...
parser.add_argument("--weight_rl_loss", type=float, default=0.4)
parser.add_argument("--weight_lm_loss", type=float, default=0.6)
...
# for layer2 in model.get_decoder().layers:
#     layer2.strat_attn.load_state_dict(layer2.encoder_attn.state_dict())
...
#dqn_parameters = list(filter(lambda p: p.requires_grad, dqn.parameters()))
total_params = sum([np.prod(p.size()) for p in model_parameters])
if args.local_rank == -1 or get_rank() == 0:
    logger.info('Number of parameter = {}'.format(total_params))

param_optimizer = list(model.named_parameters()) + list(model_emo.named_parameters())
...
#optimizer2 = Adam(dqn.parameters, lr=args.learning_rate,)
scheduler = get_linear_schedule_with_warmup(
    optimizer, num_warmup_steps=args.warmup_steps, num_training_steps=args.num_optim_steps   
)
# scheduler2 = get_linear_schedule_with_warmup(
#     optimizer2, num_warmup_steps=args.warmup_steps, num_training_steps=args.num_optim_steps   
# )
...
while True:
    ...
    for batch in train_dataloader: 
        ...
        #     tmp = []
        #     for id in batch['input_ids'][i].detach().cpu().numpy():
        #         if id+9 >= len(toker):
        #             continue
        #         tmp.append(id)
        #     context.append(toker.convert_ids_to_tokens(tmp))
        ...
        loss_agent, preds, embed = dqn.learn(batch['input_ids'], batch['attention_mask'], 
                               batch['strat_hist'], batch['sentiment_hist'], batch['reward'], 
                               batch['strat_id'], batch['utterance_num'],
                               batch['emotion'], batch['problem'], context_emo=batch['emo_encoding'])
        ...
        if step % args.gradient_accumulation_steps == 0:
            ...
            if args.num_epochs is None and global_step % args.valid_step == 0:
                if args.local_rank == -1 or get_rank() == 0:
                    ...
                    torch.save(model_emo.state_dict(), join(output_dir, f'{global_step}_emo.bin'))
