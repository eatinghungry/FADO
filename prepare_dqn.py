...
#     reader2 = pickle.load(f2)

with open(args.train_input_file) as f:
    reader = f.readlines()
with open('./_reformat/train_anno_nocon.txt') as f2:
...
def process_data(line):
    ...
    inputs, returns = inputter.convert_data_to_inputs(
        data=data,
        toker=toker,
        **kwargs
    )
...
processed_data = []
if True:#args.single_processing:
    for features in map(process_data, tqdm.tqdm(zip(reader, reader2), total=len(reader))):
        processed_data.extend(features)
else:
    with mp.Pool(processes=mp.cpu_count()) as pool:
        for features in pool.imap(process_data, tqdm.tqdm(zip(reader, reader2), total=len(reader))):
            processed_data.extend(features)

# save data
data_path = f'{save_dir}/data_dqn.pkl'
