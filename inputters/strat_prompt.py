...
from inputters.inputter_prompt import _norm, BucketSampler, BucketingDataLoader, DistributedBucketingDataLoader
...
class InputFeatures(object):
    def __init__(
        self,
        input_ids, input_ids_og, strat_hist, sentiment_hist,
        decoder_input_ids, labels, returns, ls_ids, 
        utterance_num, emotion, problem, strat_id, prompt
    ):
        self.input_ids = input_ids
        self.input_ids_og = input_ids_og
        self.input_length = len(input_ids)
        self.input_og_length = len(input_ids_og)
...
def featurize(
    ...
    utterance_num, emotion, problem, emotion_token, prompt, toker# [emotion_token_id]
):
    process = lambda x: toker.convert_tokens_to_ids(toker.tokenize(x))  
    #reverse_proc = lambda x: toker.convert_ids
    context = [c + [eos] for c in context]
    #context = [[bos] + c + [eos] for c in context]
    input_ids = sum(context, [])[:-1] #flatten
    input_ids_og = input_ids[-max_input_length:]
    #input_ids = input_ids[-(max_input_length-len(prompt)):]
    c_id = process('c: ')
    input_ids = input_ids[-(max_input_length-len(prompt)):]
    ...
    labels = (response + [eos])[:max_decoder_input_length + 1] #ground truth
...
def convert_data_to_inputs(data, toker: PreTrainedTokenizer, **kwargs):
    ...
    for i in range(len(dialog1)):
        ...
        if i > 0 and dialog1[i]['speaker'] == 'sys':
            strat_in_nlg = strat_dict[dialog1[i]['strategy']]
            prompt_txt = process(f'p: please generate a gentle response with the strategy of {strat_in_nlg}.')
            res = {
...
def convert_inputs_to_features(inputs, returns, emotion, toker, **kwargs):
    ...
    for i in range(len(inputs)):
        ...
        feat = featurize(
            ...
            ipt['utterance_num'], ipt['emotion'], ipt['problem'], emotion, ipt['prompt'],toker
        )
...
class FeatureDataset(Dataset):
    ...
    def collate(features: List[InputFeatures], toker: PreTrainedTokenizer, infer=False):
        ...
        input_ids_og = pad_sequence([torch.tensor(f.input_ids_og, dtype=torch.long) for f in features],
                          batch_first=True, padding_value=pad)
        ...
        attention_mask_og = pad_sequence([torch.tensor([1.] * f.input_og_length, dtype=torch.float) for f in features],
                          batch_first=True, padding_value=0.)
        ...
        strat_id = torch.tensor([f.strat_id for f in features], dtype=torch.long) - len(toker) + 9
        #print(strat_id)
        strat_hist = torch.tensor([f.strat_hist for f in features], dtype=torch.long)- len(toker) + 9 
        prompt_txt = [f.prompt for f in features]#torch.tensor([f.prompt for f in features], dtype=torch.long)
        res = {
            'input_ids': input_ids,
            'input_ids_og': input_ids_og,
            'last_sentence': ls_ids,
            'attention_mask_ls': attention_mask_ls,
            'attention_mask_og': attention_mask_og,
