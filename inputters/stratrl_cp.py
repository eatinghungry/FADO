...
class InputFeatures(object):
    def __init__(
        ...
        self.input_emo_ids = input_emo_ids
...
def convert_data_to_inputs(data, toker: PreTrainedTokenizer, toker2: PreTrainedTokenizer, **kwargs):
    process = lambda x: toker.convert_tokens_to_ids(toker.tokenize(x))
    process_emo = lambda x: toker2.convert_tokens_to_ids(toker2.tokenize(x))
    ...
    situation = process(_norm(problem + data1['situation']))
    ...
    sentiment_buffer = []
    usr_flag = 0
    last_sentence, last_sentence_emo = [], []
    ...
    for i in range(len(dialog1)):
        ...
        text_emo = _norm(dialog1[i]['text'])
        text = process(text)
        text_emo = process_emo(text_emo)
        ...
        if dialog1[i]['speaker'] == 'usr':
            last_sentence += [text]
            last_sentence_emo += [text_emo]
            txt_context = txt_context + [text_emo]
...
def convert_inputs_to_features(inputs, returns, emotion, toker, toker2, **kwargs):
    if len(inputs) == 0:
        return []
...
class FeatureDataset(Dataset):
    ...
    @staticmethod
    def collate(features: List[InputFeatures], toker: PreTrainedTokenizer, toker_emo:PreTrainedTokenizer, infer=False):
        pad = toker.pad_token_id
        if pad is None:
            pad = toker.eos_token_id
            assert pad is not None, 'either pad_token_id or eos_token_id should be provided'
        ...
        pad_emo = toker_emo.pad_token_id
        if pad_emo is None:
            pad_emo = toker_emo.eos_token_id
            assert pad is not None, 'either pad_token_id or eos_token_id should be provided'
        ...
        situation_ids = pad_sequence([torch.tensor(f.situation, dtype=torch.long) for f in features],
                    batch_first=True, padding_value=pad)
        situation_mask = pad_sequence([torch.tensor([1.] * f.situation_length, dtype=torch.float) for f in features],
                          batch_first=True, padding_value=0.)                                                  
        ...
        input_length_emo = torch.tensor([f.input_emo_ids_length for f in features], dtype=torch.long)
...
class DynamicBatchingLoader(object):
    ...
    def __init__(self, corpus_file, toker, toker2,batch_size, **kwargs):
    ...
    def _iter_epoch(self):
        try:
            ...
            for line in tqdm.tqdm(zip(reader, reader2), total=len(reader), desc=f"validating"):
                ...
                inputs, returns, emotion = convert_data_to_inputs(data, self.toker, self.toker2,**self.kwargs)
                features.extend(convert_inputs_to_features(inputs, returns, emotion, self.toker, self.toker2,**self.kwargs))
    ...
    def _batch_feature(self, features):
        return FeatureDataset.collate(features, self.toker, self.toker2)
...
# for inference
def prepare_infer_batch(features, toker, toker2, interact=None):
    res = FeatureDataset.collate(features, toker, toker2, True)
...
def get_infer_batch(infer_input_file, toker, toker2, **kwargs):
    ...
    for sample_id, line in tqdm.tqdm(enumerate(zip(reader, reader2)), total=len(reader), desc=f"inferring"):
        data = (json.loads(line[0]), json.loads(line[1]))
        inputs, returns, emotion = convert_data_to_inputs(data, toker, toker2,**kwargs)
        tmp_features = convert_inputs_to_features(inputs, returns, emotion, toker, toker2,**kwargs)
        for i in range(len(inputs)):
            ...
            if len(sample_ids) == infer_batch_size:
                yield prepare_infer_batch(features, toker, toker2), posts, references, sample_ids
                features = []
                sample_ids = []
                posts = []
                references = []
