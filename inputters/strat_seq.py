...
class InputFeatures(object):
    def __init__(
        ...
        self.input_length = [len(input_id) for input_id in input_ids ]
        self.strats = strats
        self.decoder_input_ids = decoder_input_ids
        self.decoder_input_length = [len(decoder_input_id) for decoder_input_id in decoder_input_ids]
        self.labels = labels
...
def featurize(
    ...
    #context = [c + [eos] for c in context] # remeber context=[[[text1]], [[text1], [text2]], ...]
    #input_ids = sum(context, [])[:-1] #flatten
    context = [list(map(lambda cc: cc+[eos], c)) for c in context] #[[[text1,eos]],[[text1,eos], [text2,eos]], ...]
    input_ids = [sum(c, [])[:-1][-max_input_length:] for c in context] #context inputs
    #input_ids = input_ids[-max_input_length:]
    #strat_id 不用 ground truth
    ## response:->List[[label1],[label2],...] 
    labels = [([strat_id[i]] + res + [eos])[:max_decoder_input_length + 1] for i, res in enumerate(response)]
    decoder_input_ids = [[bos] + lab[:-1] for lab in labels] # bos+strat_id+reponse No eos
...
def convert_data_to_inputs(data, toker: PreTrainedTokenizer, **kwargs):
    ...
    for i in range(len(dialog)):
        ...
        if i > 0 and dialog[i]['speaker'] == 'sys':
            ...
            #     'strat_id': strat_id,
            # }
            res['context'].append(context.copy()) #[[text1], [[text1],[text2]...]]
            res['response'].append(text)
            res['strat_id'].append(strat_id)
...
class FeatureDataset(Dataset):
    ...
    def collate(features: List[InputFeatures], toker: PreTrainedTokenizer, infer=False):
        ...
        temp = [torch.tensor(ff, dtype=torch.long)  for f in features for ff in f.input_ids] #features are a batch of dialog, so f is a dialog
        input_ids = pad_sequence(temp,
                          batch_first=True, padding_value=pad)
        attention_mask = pad_sequence([torch.tensor([1.] * input_len, dtype=torch.float) for f in features for input_len in f.input_length], # f.input_length is a list of length of each turn
                          batch_first=True, padding_value=0.)
        input_length = torch.tensor([input_len for f in features for input_len in f.input_length], dtype=torch.long)
        
        if not infer:
            decoder_input_ids = pad_sequence([torch.tensor(ff, dtype=torch.long) for f in features for ff in f.decoder_input_ids ],
                              batch_first=True, padding_value=pad)
            labels = pad_sequence([torch.tensor(ff, dtype=torch.long) for f in features for ff in f.labels],
