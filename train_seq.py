...
while True:
    ...
    for batch in train_dataloader: 
        ...
        for i in range(len(batch['decoder_input_ids'])):
            ...
            strat_id_pred, log_prob = pred_strat(mini_batch['input_ids'], mini_batch['strat_id'][:i])
            #reward = compute_direct_reward(strat_id_pred, batch['labels'][i][0])
            loss_strat = CrossEntropyLoss(strat_id_pred, strat_id)
            outputs = model(**mini_batch) # Seq2SeqLMOutput
            loss = outputs.pop('all') 
            ppl = outputs.pop('ppl')
            loss = weigth1*loss + weight2*loss_strat
            #buffer.add(log_prob, reward, loss)
